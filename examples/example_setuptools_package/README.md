# example_setuptools_package

This example shows how a pure Python package can install a ROS-style resource into `share/` so
`resolve_robotics_uri_py` can find it with `package://example_setuptools_package/cube.urdf`.
...
```python
resolve_robotics_uri_py.resolve_robotics_uri("package://example_setuptools_package/cube.urdf")
```

The installed file layout includes `share/example_setuptools_package/cube.urdf`.
